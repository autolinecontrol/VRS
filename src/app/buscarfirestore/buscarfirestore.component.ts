...
@Component({
  ...
  styleUrls: ['./buscarfirestore.component.scss']
})
export class BuscarfirestoreComponent implements OnInit {
  ...
  ngOnInit() {
   ...
   buscar(){
    ...
    this.db.collection('Usuarios',ref => ref.where('uid', '==', identificacion)).get().subscribe((resultado)=>{
       resultado.empty
      if(resultado.empty){
